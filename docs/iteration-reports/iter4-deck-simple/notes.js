...
8: `방금 그 Use Case 다이어그램을 **크게 확대**한 화면입니다.

여기서는 개별 유스케이스를 하나씩 짚어 가며 설명드리겠습니다. (예약 검색, 좌석 선택, 결제, 환불, 마일리지 차감 같은 주요 흐름이 어떤 액터와 연결돼 있는지 화면을 보며 안내)`,

9: `다음은 **Sequence 다이어그램**입니다. 예약, 결제, 발권으로 이어지는 핵심 시나리오를 시간순으로 보여 줍니다.
...
45: `여기까지입니다. 정리하면, **대한항공 예약 시스템**에 **DP#2 Strategy부터 DP#9 Decorator까지, 그리고 State 구현**을 하나의 실제 도메인 위에 통합했습니다.

들어 주셔서 감사합니다. **질문 있으면 편하게 말씀해 주세요.**`,

46: `(부록 — 예상 질문 대비) "왜 **Reservation** 클래스가 이렇게 비대한가?"라는 질문이 나오면 이렇게 답하면 됩니다.
